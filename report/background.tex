...
\end{equation}
where $\mathbf{delta}$ is a small step in $\mathbb{R}^5$, and $J$ is the \emph{Jacobian}, defined by:
Jacobian:
...
The Gauss-Newton Algorithm \cite{nocedal2006} solves this exact kind of problem. 
...
Because we possess $F$ we can generate unlimited training data, a collection of labelled pairs, $(x_i,y_i)$. The goal of training is then to minimise the \emph{loss function} with respect to the parameters, $w$:
