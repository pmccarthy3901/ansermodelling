...
Anser EMT \cite{jaeger2017} is the first fully open-source electromagnetic tracking platform for image-guided interventions. It uses magnetic fields to work out the position and orientation of a sensor embedded in the field. It has two main components: a planar field generator, and a tracking sensor coil. 
...
8 of these coils are positioned on the PCB, as seen in Figure \ref{fig:field_coils}
\missingfig{Figure of 8 coil generator}

\subsection{The Tracking Sensor Coil}
The tracking sensor records the flux, and due to the emitter coils running at different frequenies, is able to recover the 8 flux values due to each of the emitter coils after demodulation. These 8 fluxes are fed into the pose solving algorithm to determine the position and orientation of the sensor.

\section{Magnetic Field Derivation}
Maxwell's equations in free space are linear \cite[p.~59]{griffiths2013} so the total magnetic field will be the sum of each individual straight line. Our main goal then, is to find the magnetic field generated at an observer point $p$ due to the current flowing along $\mathbf{a}$.  
...
\begin{equation}
  \mathbf{B} = \frac{\mu_0 I}{4\pi} \int \frac{d{l}\times \mathbf{r}}{r^3}  
\end{equation}

To work out this integral we first parametrise $\mathbf{r}$ in a parameter $t$, yielding:
\begin{align*}
...
\begin{align*}
  \sqrt{\delta + \left( 1 + \frac{\beta}{a^2} \right)^2} &= \sqrt{\frac{c^2}{a^2} - \frac{\beta^2}{a^4} + 1 + 2\frac{\beta}{a^2} + \frac{\beta^2}{a^4}} \\
                             ...
                             &= \frac{1}{a} \left \vert {a} - {c} \right \vert \\
                             &= \frac{b}{a}
\end{align*}
...
\begin{equation}
\mathbf{B}_{\text{total}} = \sum_{\text{coils}} \sum_{\text{filaments}}  {B}_{\text{filament}} 
\end{equation}
Now, since we are in free space we use the identity $\mathbf{H} = \frac{{B}}{\mu_0}$ \cite[p.~285]{griffiths2013}
...
Since this maps from a subset of $\mathbb{R}^5$ to $\mathbb{R}^8$ the image of a 5-dimesional domain cannot fill $\mathbb{R}^8$, and cannot be surjective.
...
Given a function $f : \mathbb{R}^N \rightarrow \mathbb{R}^M$ with $m \geq n$, and a given $y \in \mathbb{R}^M$ suppose we want to find an $x \in \mathbb{R}^N$ that satisfies $y = f(x)$. In the case $m = n$ we might get bijectivity which would allow us to simply invert the map, however, in our case we have $n = 5$ and $m = 8$ so we cannot. In this case we have to use the inverse function theorem to get a local inverse. This leads us to iterative methods.

\subsection{Gauss Newton}
The Gauss-Newton Algorithm solves this exact kind of problem. Let's break it down. 
...
\end{equation}
Suppose now we take a step, $\delta \in \mathbb{R}^5$, we want to find the $\delta$ which minimises the cost function.
\begin{equation}
  F(x + \delta) = \frac{1}{2} \left\Vert r(x + \delta) \right\Vert^2  = \frac{1}{2} \left(r(x) + J(x)\delta \right)^\top \left(r(x) + J(x) \delta \right)
\end{equation}
Now, linearising:
\begin{equation}
...
\begin{align}
  ...
  \delta_{GD} &= -\eta J^T r
\end{align}
Now, letting $\lambda = \frac{1}{\eta}$, multiplying by $I \in R^{n \times n}$, and interpolating between these two yields:
\begin{equation}
  \label{eq:levenberg}
  \left(J^\top J + \lambda I \right)\delta = -J^\top r
\end{equation}
Here, $\lambda$ is known as the damping factor, and another key insight Levenberg had was to vary lambda dynamically. Since GN works better the closer to the solution we get and GD works better the further from the solution we are, the damping varied.\\
...
We use a fully-connected feed forward neural network (FFNN) to approximate the inverse. Suppose we have a function $\Phi: \mathbb{R}^N \rightarrow \mathbb{R}^M$ and we would like to approximate its inverse $f:= \Phi^{-1}$. We would like to find $y \in \mathbb{R}^N$ such that $y = f(x), x \in \mathbb{R}^M$. To do so, a FFNN with $L$ hidden layers performs the following composition, with $o_0 = x$ and $o_L = \hat{y}$:
...
Because we possess $\Phi$ we can generate unlimited training data, a collection of labeled pairs, $(x_i,y_i)$. The goal of training is then to minimise the residual, or \emph{loss function} with respect to the parameters, $w$:
