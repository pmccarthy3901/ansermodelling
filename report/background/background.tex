...
We have a function $f : \mathbb{R}^n \rightarrow \mathbb{R}^m$ and we know $y \in \mathbb{R}^m$, we want to find $x \in \mathbb{R}^n$ such that $y = f(x)$. We start with an initial guess $x_0 \in \mathbb{R}^n$ and define a residual:
\begin{equation}
  r_0 = y - f(x_0)
\end{equation}
Ideally, we would find an $x$ that would minimise $r$ 
\end{document}
