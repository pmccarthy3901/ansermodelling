...
def apply_constraints(
    ...
    x : np.ndarray shape(5,)
        Pose which is to be constrined
    bounds: np.ndarray shape(5,2)
        Bounds to which the pose is to be constrained 
    ...
    folded = theta > np.pi 
    theta = np.where(folded, 2.0*np.pi - theta, theta)
    phi = np.where(folded, phi + np.pi, phi)
